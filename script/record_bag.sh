...
if [[ $record_cam -eq 1 ]]; then
   ...
   if (( $(echo "$camera_throttle_hz > 0.0" | bc -l) )); then
        ...
        eval $throttle_com
        throttle_pid=$!
        # record throttled compressed images by excluding raw images, non-throttled images, and other camera data.
        exclude_camera_topics='/.*/image_raw$|/.*color/image_raw.*$|/.*depth/image_raw.*|/.*infra.*/image_raw.*'
   fi
fi
