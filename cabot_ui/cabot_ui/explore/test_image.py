...
class GPTExplainer():
    def __init__(
        ...
        if self.mode == "semantic_map_mode":
            ...
            2. 画像の左/前/右にある物体を特定し、そのシーンを知るのに必要な情報を説明すること。
            ...
            ### すべきでないこと関するルール
            6. 「画像は……」「視点」「全体的に……」など、説明を聞くユーザにとって不自然な言葉は排除してください。
            7. ただ、説明する必要がない方向があれば（例：右側には何もない）その方向に関しては説明する必要はないです。
            8. 床、天井、壁、影、遠くにあって不明瞭な物、照明の明るさや暗さに関する説明は不要です。
            9. 方向全体やシーンの全体像についての説明は排除してください。
