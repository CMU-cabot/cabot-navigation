...
class CaBotSocialMomentumController : public nav2_core::Controller     
{
public:
  CaBotSocialMomentumController();
  ~CaBotSocialMomentumController() override;
...
private:
  ...
  rclcpp::Logger logger_ = rclcpp::get_logger("CaBotSocialMomentumController");
  ...
  double social_cost_wt_;
  double sm_interaction_angle_deg_;
  double sm_interaction_distance_;
  double sm_consistent_reward_;
  double sm_switching_penalty_;
  double sm_min_distance_eps_;
  ...
  int horizon_people_;
  int num_people_;
  mutable std::mutex rl_people_mutex_;
  ...
  std::vector<lidar_process_msgs::msg::PositionArray> getPeopleSnapshot() const;
  ...
  double calculateSocialMomentumCost(
    const std::vector<geometry_msgs::msg::PoseStamped> & sampled_trajectory,
    const std::vector<lidar_process_msgs::msg::PositionArray> & people_snapshot);
  bool isInteractingAgent(
    const geometry_msgs::msg::PoseStamped & robot_pose,
    const geometry_msgs::msg::Point & person_pos) const;
  double cross2D(double ax, double ay, double bx, double by) const;
...
#endif // CABOT_NAVIGATION2__CABOT_SOCIAL_MOMENTUM_CONTROLLER_HPP_
