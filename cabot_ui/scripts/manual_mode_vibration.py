            ...
            if accessible_mask[i, j]:  # Only consider points with significant gradient
                ...
                for (rx, ry) in ray_pixels:
                    if 0 <= rx < cw and 0 <= ry < ch:  # Ensure we are within bounds
                        ...
                        grad_angle = np.arctan2(-average_grad_x, average_grad_y)  # Angle in radians
...
class ManualModeVib(Node):
    ...
    def costmap_callback(self, msg):
        ...
        inflated_img = cv2.blur(inflated_img, (5, 5))

        local_bins = inflated_img[25, 25, :]
        
        self.radar_visualizer.update_ui(25, 25, local_bins, new_mag_map=data_f, rotation_angle=(math.degrees(robot_yaw)+90))
